...
// EEPROM will silently restore values into the wrong gestures/joints:
...
//   0xCA130002  — v2 (current): 6 gestures × 2 states × 9 joints
...
constexpr uint32_t GESTURE_EEPROM_MAGIC = 0xCA130002u;
...
// ---- Stored struct (452 bytes total) ----------------------------------------

struct GestureCalEEPROM {
    uint32_t magic;
    char     calName[CAL_NAME_LEN];
    float    values[N_GESTURES][EEPROM_N_STATES][EEPROM_N_JOINTS];
};
...
/// Returns false if the gesture/state/joint combination was not found.
