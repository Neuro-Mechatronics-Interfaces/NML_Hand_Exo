...
def clamp_finger_value(value: float) -> int:
    """Map one continuous value to a set_finger_angles field.
    ...
    if not math.isfinite(number):
        raise ValueError(f"continuous finger value must be finite, got {value!r}")
    scaled = round(number * SET_FINGER_ANGLES_MAX)
