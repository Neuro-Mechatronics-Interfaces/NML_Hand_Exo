...
class EmgCentroidDecoderGUI(QWidget):
    def __init__(self):
        ...
        # LSL outlets: retain the legacy scalar stream and publish a guarded
        # v1 control contract in parallel.  Hardware is never contacted here.
        self._lsl_outlet: object | None = None  # legacy UserIntent scalar
        self._intent_v1_outlet: object | None = None
    ...
    def _build_decoder_box(self, parent: QVBoxLayout):
        ...
        tuning_grid.addWidget(QLabel("Ï„"), 1, 2)
    ...
    def _start_publish(self):
        ...
        try:
            ...
            v1_info = StreamInfo(
                "NMLIntentV1", "NMLIntent", 4, int(1.0 / PLOT_TICK_SEC),
                "float32", "nml-emg-centroid-intent-v1",
            )
            channels = v1_info.desc().append_child("channels")
            for label in ("signed_intent", "effort", "confidence", "state_code"):
                channel = channels.append_child("channel")
                channel.append_child_value("label", label)
                channel.append_child_value("unit", "normalized")
            v1_info.desc().append_child_value("schema", "nml.intent.v1")
            v1_info.desc().append_child_value(
                "note", "confidence is a scalar decoder quality proxy; future prototype modes replace it with calibrated confidence"
            )
            self._intent_v1_outlet = StreamOutlet(v1_info)
        except Exception as exc:
    ...
    def _push_intent_sample(self, value: float):
        ...
        if self._lsl_outlet is not None:
            try:
                ...
                # v1 state codes: 0=rest, 1=active.  With the current
                # one-axis model, effort/quality are intentionally conservative
                # proxies; a future prototype decoder supplies calibrated values.
                effort = float(abs(value))
                state_code = 1.0 if effort >= 0.15 else 0.0
                if self._intent_v1_outlet is not None:
                    self._intent_v1_outlet.push_sample(
                        [float(value), effort, effort, state_code]
                    )
