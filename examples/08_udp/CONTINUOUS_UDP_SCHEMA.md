...
The reference receiver requires exactly one connected hand. It queries `status:all` before arming and refuses to enable or home when firmware reports `active_side: both`, because `set_finger_angles` is broadcast by the current firmware and cannot safely select one side when both are present.
...
If the host briefly outruns the device's reply rate, the receiver bounds the queue and drops new frames before dispatch rather than dropping an older queued frame; this preserves FIFO reply-to-sequence correlation. A consumer sees the dropped frame as a gap in the acked `sequence` values.
