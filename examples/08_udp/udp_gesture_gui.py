#!/usr/bin/env python3
"""
Manual UDP sender for examples/08_udp/udp_gesture_receiver.py.
...
bind port, so `python examples/08_udp/udp_gesture_gui.py` next to a default
`python examples/08_udp/udp_gesture_receiver.py` needs no arguments.
...
Usage:
    ...
    python examples/08_udp/udp_gesture_gui.py --host 192.168.1.50 --port 10003
    python examples/08_udp/udp_gesture_gui.py --local-port 10005
"""
...
def main(argv=None):
    parser = argparse.ArgumentParser(
        description="Tkinter panel that sends gesture integers to "
                    "examples/08_udp/udp_gesture_receiver.py."
    )
