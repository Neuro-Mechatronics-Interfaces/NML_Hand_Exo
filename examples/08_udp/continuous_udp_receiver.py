...
the device's reply rate, new frames are dropped before dispatch once the bounded
ACK queue is full; FIFO reply-to-sequence correlation is never shifted.
...
# live elsewhere now: the host scales with clamp_finger_value()
...
# The receiver refuses ambiguous dual-hand hardware before arming. These commands
# therefore apply only to the one detected connected side.
ARM_COMMANDS = ("enable:all", "home:all")
...
# this receiver sends. Older firmware is rejected at startup; there is no
# per-joint fallback because that would break one-reply-per-frame correlation.
...
def value_to_signed(value):
    ...
    """
    return clamp_finger_value(float(value))
...
class MockComm:
    ...
    def _reply_for(command):
        ...
        if head == "status":
            return "Motor status: {active_side: left};"
...
class Receiver:
    ...
    def reconnect(self, attempts=RECONNECT_ATTEMPTS):
        # Replies for commands issued before USB re-enumeration are lost. Clear
        # their correlation slots so setup replies cannot ACK old UDP frames.
        self._pending.clear()
        self._ack_addr = None
        try:
            self.comm.close()
    ...
    def handle(self, data, sender, addr=None):
        ...
        if not self._apply(targets, packet["sequence"]):
            # Backpressure or a failed serial write means no actuator command
            # was issued; do not report that pose as dispatched.
            return
    ...
    def _apply(self, targets, sequence=None):
        ...
        """
        if sequence is not None and len(self._pending) >= MAX_PENDING_ACKS:
            # Device replies are FIFO. Dropping an old queue entry would shift
            # every subsequent ACK onto the wrong sequence, so apply backpressure
            # by dropping this new frame before sending it.
            self.dropped_acks += 1
            return False
...
def read_active_side(comm):
    """Return the firmware-reported connected side, or None if unavailable."""
    try:
        comm.flush_input()
        comm.send("status:all" + LINE_TERMINATOR)
    except Exception:
        return None
    deadline = time.monotonic() + 1.0
    while time.monotonic() < deadline:
        ...
        for line in reply.splitlines():
            ...
            if marker not in line:
                continue
            side = line.split(marker, 1)[1].split(",", 1)[0]
            side = side.split("}", 1)[0].strip().lower()
            if side in {"left", "right"}:
                return side
            if side == "both":
                raise RuntimeError(
                    "both left and right hands are connected; refusing to arm "
                    "broadcast set_finger_angles commands"
                )
            return None
    return None
...
def main(argv=None):
    ...
    def apply_session_setup():
        ...
        comm.flush_input()
        active_side = read_active_side(comm)
        if active_side is None:
            raise RuntimeError(
                "device did not report a single active hand; refusing to arm "
                "broadcast continuous commands"
            )
        print(f"Active hand: {active_side}")
