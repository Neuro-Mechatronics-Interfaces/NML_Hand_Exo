...
**One OpenRB-150 controller. One Dynamixel bus. One serial port.**

Both hands (if present) share the same board and USB connection. There is no separate
left-hand board or left-hand serial port. The GUI always creates a single `HandExo`
instance connected to a single serial port, regardless of mode.
