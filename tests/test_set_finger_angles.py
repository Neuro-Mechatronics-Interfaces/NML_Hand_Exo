...
class ClampFingerValueTests(unittest.TestCase):
    def test_maps_unit_range_to_signed_hundred(self):
        self.assertEqual(clamp_finger_value(0.0), 0)
        self.assertEqual(clamp_finger_value(1.0), SET_FINGER_ANGLES_MAX)
        self.assertEqual(clamp_finger_value(-1.0), -SET_FINGER_ANGLES_MAX)

    def test_rounds_to_nearest_int(self):
        self.assertEqual(clamp_finger_value(0.534), 53)
        self.assertEqual(clamp_finger_value(-0.466), -47)

    def test_clamps_out_of_range(self):
        self.assertEqual(clamp_finger_value(2.5), SET_FINGER_ANGLES_MAX)
        self.assertEqual(clamp_finger_value(-2.5), -SET_FINGER_ANGLES_MAX)

    def test_rejects_non_finite(self):
        with self.assertRaises(ValueError):
            clamp_finger_value(float("nan"))
